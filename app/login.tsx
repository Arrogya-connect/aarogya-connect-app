...
const STRINGS: Record<Lang, any> = {
  en: {
    ...
    subtitle: "Access your Aarogya Connect profile",
    username: "Username",
  ...
  */
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!resp.ok || !json.ok) {
        // show field or form error
        const errMsg = json.error || "Login failed";
        setErrors((prev: any) => ({ ...prev, form: errMsg }));
      ...
      if (json.token) {
        await AsyncStorage.setItem("sessionToken", json.token);
      }

      // Optionally save user info if returned
      // await AsyncStorage.setItem("user", JSON.stringify(json.user || {}));

      // Navigate to dashboard (same behavior as before)
      router.replace({ pathname: "/dashboard", params: { username } });
    } catch (err: any) {
      console.error("login error", err);
      const msg = err?.message || String(err);
      setErrors((prev: any) => ({ ...prev, form: "Network error" }));
      Alert.alert("Network error", msg.includes("Network request failed")
        ? "Network request failed — check API_BASE and device/network."
        : msg);
    }
  };
